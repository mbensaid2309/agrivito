...
def associate_crop(
    field_id: str, payload: FieldCropCreate, db: Session = Depends(get_db)
) -> FieldCrop:
    return field_crop_service.create(db, field_id, payload)


@router.get("/fields/{field_id}/crop", response_model=FieldCrop)
def get_associated_crop(
    field_id: str, db: Session = Depends(get_db)
) -> FieldCrop:
    association = field_crop_service.get_for_field(db, field_id)
