...
void main() {
  test(
    'media service builds multipart upload with agricultural context',
    () async {
      ...
      expect(body, isNot(contains('user-1')));
  ...
  test('anonymous media upload uses the isolated discovery route', () async {
    late http.Request captured;
    ...
    await service.upload(
      ...
      context: const MediaUploadContext(discoverySessionId: 'session-1'),
    );

    expect(captured.url.path, '/discovery/media/upload');
    expect(latin1.decode(captured.bodyBytes), contains('session-1'));
  });
